...
def drawDataArray(data, colorArray):
    ...
    normalizedData = [i / max(data) for i in
                      data]  # to make sure the height of the bars are relative and seeable in the GUI
    ...
    for i, height in enumerate(normalizedData):
        ...
        canvas.create_rectangle(x1, y1, x2, y2, fill=colorArray[
            i])  # Creates the bar for each data value using the x1,y1 and x2,y2 coordinates
...
def GenerateNewArray():
    ...
    minValue = int(
        minValueScale.get())  # Recieves the string that the user put in the Min Value box and converts it to an int
    maxValue = int(
        maxValueScale.get())  # Recieves the string that the user put in the Max Value box and converts it to an int
    arraySize = int(
        arraySizeScale.get())  # Recieves the string that the user put in the Array Size box and converts it to an int
...
Label(UserInterfaceFrame, text="Sorting Algorithm: ", bg='snow4').grid(row=0, column=0, padx=5, pady=5,
...
sortingSpeedScale = Scale(UserInterfaceFrame, from_=0.1, to=5.0, length=200, digits=2, resolution=0.1,
                          orient=HORIZONTAL, label="Sorting Speed [sec]")
sortingSpeedScale.grid(row=0, column=2, padx=5, pady=5)

# Button to Start Sorting Algorithm
Button(UserInterfaceFrame, text="Start Sorting Algorithm", command=StartSortingAlgorithm, bg='white').grid(row=0,
                                                                                                           column=3,
                                                                                                           padx=5,
                                                                                                           pady=5)
...
Button(UserInterfaceFrame, text="Generate New Random Array", command=GenerateNewArray, bg='white').grid(row=1, column=3,
                                                                                                        padx=5, pady=5)
